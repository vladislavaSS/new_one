...
module reg_map#(
    ...
    task t_axil_init; 
        begin
            s_axil.awready  <= 0; //slave is ready to accept an address
            s_axil.wready   <= 0; //slave is ready to accept data
            s_axil.bvalid   <= 0; //valid write response
            s_axil.arready  <= 0; //master ready
            s_axil.rvalid   <= 0; //
            s_axil.bresp    <= 0; //answer slave
            //s_axil.rdata    <= 0;
        end
    endtask : t_axil_init
